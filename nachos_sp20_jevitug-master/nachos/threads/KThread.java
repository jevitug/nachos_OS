...
public class KThread {
	...
	public KThread() {
		if (currentThread != null) {
			tcb = new TCB();
			//adding ThreadQueue to constructor for join()
	...
	public static void finish() {
		...
		//adding code for 'join()' to unblock the original thread
		...
		while(temp != null){
			...
			//temp.ready();
			//waitingThreads.nextThread().ready();
			if(temp != null){
				temp.ready();
				temp = waitingThreads.nextThread();
			}
		...
		//END code for 'join()' implementation
	...
	public void join() {
		...
		//if this is also current thread or this has already called join, assert
		Lib.assertTrue(this != currentThread, "this can't be currentThread!");
		//Lib.assertTrue(this.isJoinable != false, "Join has already been called!");
		//if this has already finished, enable interrupts and return
		if(this.status == statusFinished || this.isJoinable == false){
			Machine.interrupt().restore(currStatus);
			return;
		}
		else{
			//add currentThread to wait queue
			waitingThreads.waitForAccess(currentThread);
			//set isJoinable to false so this thread cannot call join again
	...
	public boolean isJoinable = true;
	...
	private static void joinTest1(){
		...
		Lib.assertTrue((child1.status == statusFinished), " Excpected child1 to be finished.");
	}

	private static void joinTest2(){
		KThread child0 = new KThread(new Runnable() {
			public void run(){
				System.out.println("I (heart) Nachos!");
			}
		});
		child0.setName("child0").fork();
		for(int i = 0; i < 5; i++){
			System.out.println("busy...");
			KThread.currentThread().yield();
		}
		System.out.println("calling child0.join() twice, assertion should be thrown");
		child0.join();
		child0.join();	
		System.out.println("post second call of child0.join()");
	}
